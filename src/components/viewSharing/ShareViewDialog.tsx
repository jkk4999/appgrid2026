 ...
 *
 * Dialog for sharing a view with other users.
 * Features:
...
// Types
import type { ShareableUser } from '../../store/slices/viewSharingSlice';

import type { UserProfile, SharedViewDTO } from '../../sObjectMetadataTypes';
...
const ShareViewDialog: React.FC<ShareViewDialogProps> = ({ apiClient }) => {
  /*==========================================
    ** CONTEXT
    ==========================================*/
  ...
  const {
    ...
    shareDialogViewName,
    shareDialogUsers,
    shareDialogUsersLoading,
    shareDialogProfileFilter,
    closeShareViewDialog,
    setShareDialogUsers,
    setShareDialogUsersLoading,
    setShareDialogProfileFilter,
  } = useStore(
    useShallow((state) => ({
      showShareViewDialog: state.showShareViewDialog,
      shareDialogViewId: state.shareDialogViewId,
      shareDialogViewName: state.shareDialogViewName,
      shareDialogUsers: state.shareDialogUsers,
      shareDialogUsersLoading: state.shareDialogUsersLoading,
      shareDialogProfileFilter: state.shareDialogProfileFilter,
      closeShareViewDialog: state.closeShareViewDialog,
      setShareDialogUsers: state.setShareDialogUsers,
      setShareDialogUsersLoading: state.setShareDialogUsersLoading,
      setShareDialogProfileFilter: state.setShareDialogProfileFilter,
    }))
  );
  ...
  useEffect(() => {
    ...
    const loadProfiles = async () => {
      try {
        ...
        prettyPrint('ShareViewDialog loadProfiles error', err);
      } finally {
    ...
    const loadUsers = async () => {
      if (!shareDialogViewId) return;

      try {
        setShareDialogUsersLoading(true);
        ...
        const recipientsResponse = await apiClient.getViewShareRecipients({
          viewId: shareDialogViewId,
        });

        if (recipientsResponse && Array.isArray(recipientsResponse)) {
          const sharedUserIds = new Set(
            recipientsResponse.map((r: SharedViewDTO) => r.sharedWithId).filter(Boolean)
          );

          // Check if shared with all
          const hasAllUsersShare = recipientsResponse.some(
            (r: SharedViewDTO) => r.shareType === 'All Users'
          );
        ...
        prettyPrint('ShareViewDialog loadUsers error', {
          fullError: err,
        ...
        setShareDialogUsersLoading(false);
      }
    };

    if (showShareViewDialog) {
      loadProfiles();
      loadUsers();
  ...
  }, [apiClient, setShareDialogUsers, setShareDialogUsersLoading, shareDialogViewId, showShareViewDialog]);

  // Filter users when profile filter changes
  useEffect(() => {
    if (shareDialogProfileFilter) {
      const filtered = allUsers.filter(
        (user) => user.profileId === shareDialogProfileFilter
      );
      setShareDialogUsers(filtered);
    } else {
      setShareDialogUsers(allUsers);
    }
  }, [shareDialogProfileFilter, allUsers, setShareDialogUsers]);
  ...
  const onSelectionChanged = useCallback(
    (event: SelectionChangedEvent) => {
      ...
      const updatedUsers = shareDialogUsers.map((user) => ({
        ...user,
      ...
      const updatedAllUsers = allUsers.map((user) => ({
        ...user,
        isSelected: selectedIds.has(user.id) || (shareDialogProfileFilter && !updatedUsers.find(u => u.id === user.id) ? user.isSelected : selectedIds.has(user.id)),
      }));

      setAllUsers(updatedAllUsers);
    },
    [shareDialogUsers, allUsers, shareDialogProfileFilter]
  );
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      prettyPrint('ShareViewDialog handleShare error', err);
    ...
    >
      ...
      >
        <Box display="flex" alignItems="center" gap={1}>
          <ShareIcon />
          <Typography variant="h6">Share View: {shareDialogViewName}</Typography>
        </Box>
        <IconButton
          onClick={handleClose}
          disabled={saving}
      ...
      >
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <Autocomplete
            ...
            value={profiles.find((p) => p.id === shareDialogProfileFilter) || null}
            onChange={(_, newValue) =>
              setShareDialogProfileFilter(newValue?.id || null)
            }
        ...
        <Typography variant="body2" color="text.secondary">
          {shareWithAll
            ? 'This view will be shared with all users in the organization'
        ...
        {/* Success Display */}
        {success && (
          <Alert severity="success">View shared successfully!</Alert>
        )}
      </DialogContent>
